...
// Logout Button Functionality
document.getElementById("logoutBtntrangchu").onclick = function () {
    var confirmation = confirm("Bạn có chắc chắn muốn về trang chủ không?");
    if (confirmation) {
        document.querySelector('#logoutForm').submit();  // Submit logout form to PHP
    }
};
...
function togglePasswordVisibility(inputId, icon) {
    ...
    const iconClass = input.type === 'password' ? 'fa-eye-slash' : 'fa-eye';
    input.type = input.type === 'password' ? 'text' : 'password';
    icon.className = 'fa ' + iconClass + ' toggle-password';
}